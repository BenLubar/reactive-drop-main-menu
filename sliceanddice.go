...
func main() {
	...
	for i := 0; ; i++ {
		...
		for _, a := range areas {
			for _, f := range a.frames {
				...
				sequences = append(sequences, sequence{name, dst})
			}
		}
	}
	...
	bestSquareness, bestSize := 1<<30, 1<<30
	...
	for tryWidth := 1 << 22; tryWidth >= 4; tryWidth >>= 1 {
		for _, sortMethod := range sortMethods {
			...
			tex, sheetData := packSheet(sequences, sequenceOrder, tryWidth)
			if tex == nil {
				continue
			}

			width, height := tex.Rect.Dx(), tex.Rect.Dy()
			...
			if (size < bestSize) || (size == bestSize && squareness < bestSquareness) {
				bestTexture = tex
			...
			fmt.Printf("Packing option: %dx%d (%d pixels) (%s)\n", tex.Rect.Dx(), tex.Rect.Dy(), tex.Rect.Dx()*tex.Rect.Dy(), message)
		}
	}
	...
	out, err := os.Create("main_menu_sheet.tga")
	if err != nil {
		panic(err)
	}
	...
	err = os.WriteFile("main_menu_sheet.sht", bestSheetData, 0644)
	if err != nil {
		panic(err)
	}

	enum, err := os.Create("main_menu_sheet_enum.txt")
	if err != nil {
		panic(err)
	}

	fmt.Fprintf(enum, "\tDECLARE_HUD_SHEET( MainMenuSheet )\n")
	for _, s := range sequences {
		fmt.Fprintf(enum, "\t\tDECLARE_HUD_SHEET_UV( %s ),\n", s.name)
	}
	fmt.Fprintf(enum, "\tEND_HUD_SHEET( MainMenuSheet );\n")
	...
	cmd := exec.Command(`D:\Program Files\Steam\steamapps\common\Alien Swarm Reactive Drop\bin\vtex.exe`, `-nopause`, `-nop4`, `-game`, `D:\Program Files\Steam\steamapps\common\Alien Swarm Reactive Drop\reactivedrop`, `-outdir`, `.`, `main_menu_sheet`)
...
func packSheet(sequences []sequence, sequenceOrder []int, width int) (*image.NRGBA, []byte) {
	...
	for _, i := range sequenceOrder {
		...
		if row+seq.img.Rect.Dy() > nextRow {
			nextRow = row + seq.img.Rect.Dy()
		}
